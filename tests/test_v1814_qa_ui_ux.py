...
class TestVhostPolicyVersion:
    ...
    def test_title_shows_1814(self):
        ...
        assert "AntiBotWaf_GW_1.9.10" in src, "Page title must contain AntiBotWaf_GW_1.9.10"
...
class TestAgentsVersion:
    ...
    def test_title_shows_1814(self):
        ...
        assert "AntiBotWaf_GW_1.9.10" in src
...
class TestCrossDashboardVersion:
    ...
    def test_sidebar_brand_ver_1814(self, dashboard):
        ...
        import config as _cfg
        _ver = _cfg.GW_VERSION.rsplit("_", 1)[1]   # "AntiBotWaf_GW_1.9.10" → "1.9.4"
        src = _read(dashboard)
        assert re.search(
            r'id="sidebar-brand-ver"[^>]*>\s*' + re.escape(_ver) + r'\s*<', src), \
