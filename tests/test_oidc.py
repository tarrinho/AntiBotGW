...
async def _fake_keycloak(token_status: int = 200, token_resp: dict | None = None,
    ...
    port = site._server.sockets[0].getsockname()[1]
    try:
        # INT4-10 (1.8.13): stub must carry sub so the unconditional sub-binding
        # check in the callback passes. Flow tests use "sub123" to match the
        # default userinfo_resp sub. Real JWKS/RS256 verification is covered in
        # test_v1811_oidc_idtoken_verify.py.
        with patch("admin.oidc._verify_id_token",
                   AsyncMock(return_value={"sub": "sub123"})):
            yield f"http://127.0.0.1:{port}/realms/test"
    finally:
        await runner.cleanup()


def _valid_state(next_url: str = "/antibot-appsec-gateway/secured/control-center",
...
async def test_d16_new_user_auto_provisioned():
    ...
    async with _fake_keycloak() as issuer:
        async with _patched_oidc(OIDC_ENABLED=True, OIDC_ISSUER=issuer,
            ...
            with patch("db.open_conn", return_value=mock_conn), \
