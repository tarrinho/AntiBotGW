...
def test_brand_version_current(name):
    h = _html(name)
    # contract change: brand version tracks GW_VERSION (config.py), now 1.9.5;
    # bump-version.sh keeps sidebar-brand-ver in sync. Stale anchor was 1.8.14.
    assert '<div id="sidebar-brand-ver">1.9.5</div>' in h, \
        f"{name}: brand version not 1.9.5"
