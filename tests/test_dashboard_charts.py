...
# ── service.html: traffic share by vhost ──────────────────────────────────────

def test_service_vhost_share_card_exists():
    """service.html must contain the vhost share card with its container div."""
    src = _read("service.html")
    assert "svc-vhost-share" in src, (
        "service.html: missing #svc-vhost-share div — vhost traffic share card not present."
    )

def test_service_vhost_share_fetches_vhost_stats():
    """loadVhostShare() must fetch from the vhost-stats endpoint."""
    src = _read("service.html")
    fn_start = src.find("function loadVhostShare(")
    assert fn_start != -1, "service.html: loadVhostShare() function not found"
    block = src[fn_start: fn_start + 800]
    assert "vhost-stats" in block, (
        "service.html: loadVhostShare() must fetch from /vhost-stats endpoint."
    )

def test_service_vhost_share_escapes_hostname():
    """loadVhostShare() must call escapeHtml on hostname before injecting into innerHTML."""
    src = _read("service.html")
    fn_start = src.find("function loadVhostShare(")
    assert fn_start != -1, "service.html: loadVhostShare() function not found"
    block = src[fn_start: fn_start + 2000]
    assert "escapeHtml(v.hostname)" in block, (
        "service.html: loadVhostShare() must wrap v.hostname with escapeHtml() before innerHTML."
    )

def test_service_vhost_share_win_toggle_buttons():
    """service.html must have 1h and 24h toggle buttons for the vhost share window."""
    src = _read("service.html")
    assert "vhost-win-1h" in src, "service.html: missing #vhost-win-1h toggle button"
    assert "vhost-win-24h" in src, "service.html: missing #vhost-win-24h toggle button"
    assert "setVhostWin" in src, "service.html: missing setVhostWin() function"

def test_service_vhost_share_computes_percentage():
    """loadVhostShare() must compute a percentage from the total."""
    src = _read("service.html")
    fn_start = src.find("function loadVhostShare(")
    assert fn_start != -1, "service.html: loadVhostShare() function not found"
    block = src[fn_start: fn_start + 2000]
    assert "total" in block and "%" in block, (
        "service.html: loadVhostShare() must compute a percentage (share / total × 100)."
    )

def test_service_vhost_share_interval_registered():
    """loadVhostShare must be scheduled via _timers.push(setInterval(...))."""
    src = _read("service.html")
    assert "setInterval(loadVhostShare" in src, (
        "service.html: loadVhostShare not scheduled via setInterval — it will only load once."
    )
    assert "_timers.push(setInterval(loadVhostShare" in src, (
        "service.html: setInterval(loadVhostShare) not tracked in _timers — interval will leak."
    )


# ── agents.html: IP intel risk breakdown ────────────────────────────────────

def test_agents_ip_intel_risk_breakdown_section_exists():
    """fetchIpIntel() must compute riskBreakdownHtml from it.risk_breakdown."""
    src = _read("agents.html")
    assert "riskBreakdownHtml" in src, (
        "agents.html: missing riskBreakdownHtml variable in fetchIpIntel()"
    )

def test_agents_ip_intel_risk_breakdown_uses_rsn_class():
    """Risk breakdown bars must use the .rsn class for consistent styling."""
    src = _read("agents.html")
    bd_start = src.find("riskBreakdownHtml")
    assert bd_start != -1
    block = src[bd_start: bd_start + 600]
    assert 'class="rsn"' in block, (
        "agents.html: risk breakdown bars must use class=\"rsn\" for popover styling"
    )

def test_agents_ip_intel_risk_breakdown_escapes_reason():
    """Risk breakdown must call escapeHtml() on the reason string."""
    src = _read("agents.html")
    bd_start = src.find("riskBreakdownHtml")
    assert bd_start != -1
    block = src[bd_start: bd_start + 600]
    assert "escapeHtml(r)" in block, (
        "agents.html: risk breakdown must escape reason string with escapeHtml(r)"
    )

def test_agents_ip_intel_risk_breakdown_embedded_in_return():
    """riskBreakdownHtml must be embedded in the fetchIpIntel() return string."""
    src = _read("agents.html")
...
def test_agents_ip_intel_risk_breakdown_label():
    """Breakdown section must have a visible label for the user."""
    src = _read("agents.html")
    assert "Score breakdown" in src, (
        "agents.html: missing 'Score breakdown' heading in risk breakdown section"
    )
