...
# 1.9.1 iter-10 cleanup — explicit `__all__` declares the re-export contract
# so ruff stops flagging these as unused. The previous pattern relied on
# per-line lint-suppression comments which were noisy and easy to forget on
# the next addition. The list is authoritative for `from db import *` and
# documents the public surface for callers.
__all__ = [
    # sqlite
    "db_init", "db_writer_loop", "db_load_secrets", "db_load_config",
    "db_load_state", "_SECRET_KEYS", "_SCHEMA_MIGRATIONS",
    "_apply_sqlite_migrations", "_refresh_integration_state",
    "check_ip_ban", "check_ip_ban_vhost",
    "check_ip_ban_cached", "check_ip_ban_vhost_cached",
    "prune_ip_bans", "prune_old_events",
    # postgres
    "db_init_postgres", "pg_insert_event", "pg_db_size", "pg_test_roundtrip",
    "_pg_mirror_kv", "_migrate_recent_events", "_apply_pg_migrations",
    "_postgres_load_module",
    # conn helper
    "open_conn", "active_backend", "PgUnavailableError",
    # local module-level functions
    "db_read_events", "db_read_events_async", "db_health_snapshot",
]
...
# Phase 3 — backend-aware connection helper for ad-hoc reads.
# N1 fix: do NOT re-export the `conn` context manager into the package
# namespace — its name collides with the `db.conn` MODULE and the
# re-export silently shadows the module. Callers must explicitly
# `from db.conn import conn` for the context manager. The non-collision
# names (open_conn, active_backend, PgUnavailableError) are still
# re-exported for ergonomic access via `db.open_conn(...)`.
from db.conn import open_conn, active_backend, PgUnavailableError  # noqa: F401
