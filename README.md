...
docker pull  registry.example.com/appsecgw/appsecgw:1.8.11
```
...
| Version | Highlights |
|---|---|
| **1.8.11** | **Security-hardening release** (from a full security review; no UI changes). **(Security)** WAF now scans the full request body via `WAF_BODY_SCAN_BYTES` (was 64 KiB — padding bypass, **H1**); central CSRF gate in `protect()` for every admin mutation (**H2**); honey-cred probe rate-limited + bans the requester not the victim (**H3**); `agw_csrf` scoped to the admin namespace so upstream XSS can't read it (**M1**); `ALLOW_PRIVATE_UPSTREAM` defaults **OFF** — SSRF guard on (**M2**); OIDC id_token signature verified against the issuer JWKS, RS256-only, reject `none`/HS\* (**M3**); PoW min-solve-time floor fixed + single-use token (**M4**); GraphQL body window + bounded depth scan (**M5**); session `source_ip`/idle-timeout survive restart for `BIND_SESSION_TO_IP` (**M7**). **(Changed)** new `PyJWT` dependency (OIDC only, lazy); `ALLOW_PRIVATE_UPSTREAM` `1`→`0`. **(Deferred)** removing CSP `script-src 'unsafe-inline'` (needs an inline-handler refactor — follow-up). **(Tests)** `test_v1811_security.py` (20); conftest `_auto_attach_csrf_header` fixture mirrors the dashboard CSRF shim for the test client. |
