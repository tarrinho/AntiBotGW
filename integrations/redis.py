...
def _hmac_sign(value: str) -> str:
    """Append a 16-char HMAC-SHA256 signature to a ban value string."""
    if not _REDIS_HMAC_KEY:
        return value
    sig = _hmac.new(_REDIS_HMAC_KEY, value.encode(), hashlib.sha256).hexdigest()[:16]
    return f"{value}|{sig}"
...
def _hmac_verify(raw: str) -> str | None:
    ...
    if len(parts) != 2:
        return None
    value, sig = parts
    expected = _hmac.new(_REDIS_HMAC_KEY, value.encode(), hashlib.sha256).hexdigest()[:16]
