...
- **`secrets_kv` / Postgres schema missing when DSN configured post-startup** (`core/proxy_handler.py`): If the container started with no `POSTGRES_DSN` (or an unreachable one), `db_init_postgres()` returned early and the schema tables (`secrets_kv`, `config_kv`, etc.) were never created. Any later `/secrets` save of `POSTGRES_DSN` would immediately try to mirror the secret into `secrets_kv` and fail with `UndefinedTable`. Fixed in two places: (1) `secrets_endpoint` — after propagating a new `POSTGRES_DSN`, fires `db_init_postgres()` in a background executor; (2) `db_switch_endpoint` — `await`s `db_init_postgres()` before `_migrate_recent_events` so migration threads never race against a missing schema. Both calls are idempotent (`CREATE TABLE IF NOT EXISTS`).
