...
### Added
- **AWS ELB / ALB health check pass-through** (`config.py`, `core/proxy_handler.py`) — AWS Application/Network Load Balancers send `GET <path>` with `User-Agent: ELB-HealthChecker/2.0` and only `Host`, `Connection: close`, `Accept-Encoding` headers — no `Accept`, `Accept-Language`, or `Sec-Fetch-*`. This triggered `ua-non-browser` (+25) and `ai-headers-incomplete` (+20) on every probe; after two requests the LB node accumulated 90+ risk points, was banned, and the target group was marked unhealthy causing traffic drains. Fix: new two-factor bypass guard in `protect()` — when **both** `ELB_HEALTH_CHECK_PATH` and `ELB_HEALTH_CHECK_UA` match, the request short-circuits the entire detection pipeline and returns `200 ok` immediately. Neither condition alone is sufficient (path-only would allow external clients to bypass via URL; UA-only would allow spoofed UAs from any IP). The health check path is operator-controlled and must match the value set in the AWS target-group console — it is never echoed in responses or logs (only a SHA-256[:8] hash is logged). ELB nodes arrive via VPC private addresses on a trusted-proxy chain, blocking external IP-spoofing. New env vars: `ELB_HEALTH_CHECK_PATH` (default: empty = bypass disabled), `ELB_HEALTH_CHECK_UA` (default: `ELB-HealthChecker`).
